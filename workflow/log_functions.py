...
def actlog(content):
    ...
    a = JsonActlog.objects.create(session=ts, json_lookup_id=jli, json_file_id=jfi, activitylog=content+tag)
    a.save()
...
def errorlog(content):
    ...
    jli = gvars.ingest_data_lookup_id
    jfi = gvars.ingest_data_file_id
    e = JsonErrors.objects.create(session=ts, json_lookup_id=jli, json_file_id=jfi, errorcode=content+tag)
    e.save()
